...
export default function DropdownFullWidth({
  options,
      ...
      <Listbox value={selected} onChange={setSelected}>
        <div className="relative mt-1">
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="absolute left-10 z-10 mt-3 max-h-60  w-full -translate-x-12 overflow-auto rounded-md bg-white text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
              {options.map((option) => (
                <Listbox.Option
                  key={uuidv4()}
                  className={({ active }) =>
